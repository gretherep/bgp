...
import { Languages } from "lucide-react"; // Opcional: para un icono sutil
...
export default function MediaModal() {
  ...
  // ✅ Nuevo estilo para el Idioma (Color Naranja/Ámbar)
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={closeModal}>
      ...
      >
        <button onClick={closeModal} className="absolute top-4 right-4 w-9 h-9 rounded-full bg-black/40 hover:bg-black/60 flex items-center justify-center z-10 text-white text-xl font-bold">✕</button>
        ...
        <div className="p-6 flex flex-col md:flex-row gap-6 overflow-y-auto max-h-[85vh]">
          ...
          <div className="flex-1 flex flex-col gap-4 min-w-0">
            <h2 className="text-2xl md:text-3xl font-bold leading-tight">{selectedMedia.title}</h2>

            {/* Tags */}
            <div className="flex flex-wrap gap-2">
              <span className="px-3 py-1 rounded-full text-xs font-semibold uppercase tracking-wide" style={{ backgroundColor: categoryStyle.bg, border: `1px solid ${categoryStyle.border}`, color: "#fff" }}>
                {selectedMedia.category}
              </span>

              {/* ✅ MOSTRAR IDIOMA SI EXISTE */}
{selectedMedia.idioma && selectedMedia.idioma.trim() !== "" && (
  <span 
    ...
    style={{ backgroundColor: languageStyle.bg, border: `1px solid ${languageStyle.border}`, color: "#fff" }}
  >
    ...
    {selectedMedia.idioma}
              ...
              {genres.map((genre, index) => {
                ...
                return (
                  <span key={genre} className="px-3 py-1 rounded-full text-xs font-medium transition-transform hover:scale-105" style={{ backgroundColor: color.bg, border: `1px solid ${color.border}`, color: "#fff" }}>
                    {genre}
                  </span>
                );
              })}

              <span className="px-3 py-1 rounded-full text-xs font-medium" style={{ backgroundColor: yearStyle.bg, border: `1px solid ${yearStyle.border}`, color: "#fff" }}>
                {selectedMedia.year}
              </span>
            </div>

            {/* RATING INTERACTIVO (Se mantiene igual) */}
            <div className="flex items-center space-x-0.5 group">
              {Array.from({ length: 5 }, (_, i) => {
                ...
                return (
                  ...
                  >
                    <svg className={`w-5 h-5 transition-all duration-200 ${showFilled ? "text-yellow-400 drop-shadow-[0_0_8px_rgba(250,204,21,0.5)]" : "text-white/20"}`} fill={showFilled ? "currentColor" : "none"} stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24">
            ...
            <div className="flex-1 min-h-0">
              <h3 className="text-xs uppercase text-[var(--color-accent)] mb-2">Sinopsis</h3>
